...
# Module API #
The Module API is modeled after [CommonJS Modules](http://wiki.commonjs.org/wiki/Modules/1.1.1) is available but currently only supports the built-in modules:
...
```javascript
var SomeModuleAPI = require("SomeModuleID");
```
...
Opens the `url` and loads it to the page. Once the page is loaded, the optional `callback` is called using [[`page.onLoadFinished`|#onLoadFinished]], and also provides the page status to the function (`"success"` or `"fail"`).
...
#### `release()` {void} ####
**Stability:** _DEPRECATED_ (replaced by [[close()|close() {void}]])
...
Also see [[`page.open`|#open(url, _callback_) {void}]] for an alternate hook for the `onLoadFinished` callback.
...
Using an embedded web server module called [Mongoose](http://code.google.com/p/mongoose/), PhantomJS script can start a web server.
